...
class ArtistListComponent extends React.Component {
    ...
    deleteArtistsClicked() {
        ...
        if (x.length > 0) {
            let msg;
            if (x.length > 1) {
                msg = "Please confirm the deletion of " + x.length + "artists";
            } else {
                msg = "Please confirm the deletion of the following artist: " + x[0].name;
            }
    ...
    refreshArtists(cp) {
        BackendService.retrieveAllArtists(cp, this.state.limit)
            .then(resp => {
                this.setState({
                    artists: resp.data.content, totalCount: resp.data.totalPages+1,
            ...
            <div className="m-4">
                <div className=" row my-2 mr-0">
                    ...
                    <button className="btn btn-outline-secondary ml-auto"
                            onClick={this.addArtistClicked}><FontAwesomeIcon icon={faPlus}/>{' '}Add
                    </button>
                    <button className="btn btn-outline-secondary ml-2"
                            onClick={this.deleteArtistsClicked}><FontAwesomeIcon icon={faTrash}/>{' '}Delete
                    </button>
                </div>
                <div className="row my-2 mr-0">
                    <PaginationComponent
