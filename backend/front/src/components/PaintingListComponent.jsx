...
class PaintingListComponent extends React.Component {
    ...
    deletePaintingsClicked() {
        ...
        if (x.length > 0) {
            let msg;
            if (x.length > 1) {
                msg = "Please confirm the deletion of " + x.length + "paintings";
            } else {
                msg = "Please confirm the deletion of the following painting: " + x[0].name;
            }
        ...
        BackendService.retrieveAllPaintings(cp, this.state.limit)
            .then(async (resp) => {
                ...
                for (let elem of resp.data.content) {
                    const info = await BackendService.retrieveMuseumsAndArtist(elem.artist.id, elem.museum.id);
                    data.push({artist: info[0], museum: info[1], name: elem.name, id: elem.id});
                }
                console.log(resp, 'REPS')
                this.setState({
                    paintings: data, totalCount: resp.data.totalPages+1,
            ...
            <div className="m-4">
                <div className=" row my-2 mr-0">
                    ...
                    <button className="btn btn-outline-secondary ml-auto"
                            onClick={this.addPaintingClicked}><FontAwesomeIcon icon={faPlus}/>{' '}Add
                    </button>
                    <button className="btn btn-outline-secondary ml-2"
                            onClick={this.deletePaintingsClicked}><FontAwesomeIcon icon={faTrash}/>{' '}Delete
                    </button>
                </div>
                <div className="row my-2 mr-0">
                    <PaginationComponent
