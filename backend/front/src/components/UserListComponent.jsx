...
class UsersListComponent extends React.Component {
    ...
    deleteUsersClicked() {
        ...
        if (x.length > 0) {
            let msg;
            if (x.length > 1) {
                msg = "Please confirm the deletion of " + x.length + "users";
            } else {
                msg = "Please confirm the deletion of the following user: " + x[0].login;
            }
    ...
    refreshUsers(cp) {
        ...
        BackendService.retrieveAllUsers(cp, this.state.limit)
            .then(resp => {
                ...
                this.setState({
                    users: resp.data.content, totalCount: resp.data.totalPages+1, page: cp, hidden: false });
            })
            .catch(() => {
            ...
            <div className="m-4">
                <div className=" row my-2 mr-0">
                    ...
                    <button className="btn btn-outline-secondary ml-auto"
                            onClick={this.addUserClicked}><FontAwesomeIcon icon={faPlus}/>{' '}Add
                    </button>
                    <button className="btn btn-outline-secondary ml-2"
                            onClick={this.deleteUsersClicked}><FontAwesomeIcon icon={faTrash}/>{' '}Delete
                    </button>
                </div>
                <div className="row my-2 mr-0">
                    <PaginationComponent
