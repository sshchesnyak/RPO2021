...
class MuseumListComponent extends React.Component {
    ...
    deleteMuseumsClicked() {
        ...
        if (x.length > 0) {
            let msg;
            if (x.length > 1) {
                msg = "Please confirm the deletion of " + x.length + "museums";
            } else {
                msg = "Please confirm the deletion of the following museum: " + x[0].name;
            }
    ...
    refreshMuseums(cp) {
        ...
        BackendService.retrieveAllMuseums(cp, this.state.limit)
            .then(resp => {
                ...
                this.setState({
                    museums: resp.data.content, totalCount: resp.data.totalPages+1,
